...
const SalaryPage = () => {
    ...
    useEffect(() => {
        fetch("/salary.json") // Updated path to reference the public directory
            .then(res => res.json())
            .then(data => setSalary(data))
            .catch(error => console.log("Error fetching salary data: ", error));
    }, [searchText]);
                    ...
                    salary.map((data) => (
                        <div key={data.id} className='shadow px-4 py-8'>
                            ...
                            <div className='flex flex-wrap gap-4'>
                                <a href="/" className='underline' onClick={(e) => e.preventDefault()}>{data.status}</a>
                                <a href="/" className='underline' onClick={(e) => e.preventDefault()}>{data.skills}</a>
                            </div>
                        </div>
                    ))
                }
