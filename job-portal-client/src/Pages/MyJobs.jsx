...
const MyJobs = () => { 
    ...
    useEffect(()=>{
        setIsLoading(true);
        fetch(`http://localhost:5000/myJobs/anadorothy05@gmail.com`).then((res) => res.json()).then((data) => {
            setJobs(data);
            setIsLoading(false);
        });
    },[searchText]);
    ...
    <div class="rounded-t mb-0 px-4 py-3 border-0">
      <div class="flex flex-wrap items-center">
        ...
        <div class="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          <Link to="/post-job"> <button class="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post A New Job</button>
</Link>
        </div>
      </div>
    </div>
